''' @file PyTool based cli to create dependency report
...
from edk2toollib.uefi.edk2.parsers import inf_parser, buildreport_parser, dec_parser
...
def JS(key, value, comma=True):
    r = json.JSONEncoder().encode(key.strip()) + ": " + \
        json.JSONEncoder().encode(value.strip())
    if comma:
        r = r + ","
    return r
    ...
    for a in InfObj.PackagesUsed:
        ...
        if a in PackageObjectList:
            ...
            try:
                ...
                Errors.append(
                    "Failed to Parse DEC %s.  Exception: %s" % (a, str(e)))
                continue

        # now find it
        if(GlobalString in [a.name for a in DecObj.Protocols]):
            return DecObj.Dict["PACKAGE_NAME"].strip().lower()
        if(GlobalString in [a.name for a in DecObj.PPIs]):
            return DecObj.Dict["PACKAGE_NAME"].strip().lower()
        if(GlobalString in [a.name for a in DecObj.Guids]):
            return DecObj.Dict["PACKAGE_NAME"].strip().lower()
        if(GlobalString in [a.name for a in DecObj.LibraryClasses]):
            return DecObj.Dict["PACKAGE_NAME"].strip().lower()
        if(GlobalString in [a.token_space_name + "." + a.name for a in DecObj.Pcds]):
            return DecObj.Dict["PACKAGE_NAME"].strip().lower()
...
def GetVersionOfPipDependencies():
    ''' get the version of the pip modules and return as dict '''
    versions = dict()
    for package in pkg_resources.working_set:
        version = pkg_resources.get_distribution(package).version
        logging.info(f"{package.project_name} version: {version}")
        versions[package.project_name] = version
    return versions
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Parse Module INF file and show its dependencies')
    parser.add_argument("--Workspace", dest="ws",
                        help="absolute path to Edk2 build workspace", default=None)
    parser.add_argument("--PackagesPathCsvString", dest="ppcsv",
                        help="Packages Path csv (absolute paths or ws relative paths)", default=None)
    parser.add_argument("--OutputReport", dest="OutputReport",
                        help="Path to output report", default=None)
    parser.add_argument("--InputDir", dest="InputDir",
                        help="folder to start scanning.  Default is same as Workspace", default=None)
    parser.add_argument("--BuildReport", dest="BuildReport",
                        help="optinal parameter will include graphs for as built dependencies as detected from build report", default=None)
    parser.add_argument("--IgnoreFolderCsvString", dest="IgnoreFoldersCSV",
                        help="Add folders to ignore list when parsing for InputDir", default=None)
    parser.add_argument(
        "--CodebaseName", help="Name of codebase.  Will be displayed on report", default="Test Codebase")
    parser.add_argument(
        "--CodebaseVersion", help="Version of Codebase.  Will be displayed on report", default="1.0.0")

    # Turn on dubug level logging
    parser.add_argument("--debug", action="store_true", dest="debug",
                        help="turn on debug logging level for file log",  default=False)
    # Output debug log
    parser.add_argument("-l", dest="OutputLog",
                        help="Create an output log file: ie -l out.txt", default=None)
    ...
    if(options.OutputLog):
        if(len(options.OutputLog) < 2):
            ...
            filelogger = logging.FileHandler(
                filename=options.OutputLog, mode='w')
            if(options.debug):
